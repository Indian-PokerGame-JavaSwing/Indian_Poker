...
public class IndianPokerFrame extends JFrame {
    ...
    private void initUI() {
        ...
        btnNext.addActionListener(e -> {
            ...
            lblAICard.setFrontImage(imageLoader.getCardImage(aiCard));
            lblAICard.showFront(); // 상대방은 즉시 앞면 보여야 함

            // 🛑 바로 flip하지 않고, 먼저 뒷면 고정
            lblHumanCard.showBack();

            // ⏳ 약간 딜레이 후 flip 실행 → 자연스럽게 보임
            new Timer(300, ev -> {
                lblHumanCard.flip();
    ...
    private void refreshUI() {
        ...
        if (engine.getPhase() == IndianPokerEngine.Phase.BETTING) {
            lblHumanCard.setFrontImage(imageLoader.getCardImage(humanCard));
            lblHumanCard.flip(); // 애니메이션 실행

            lblAICard.setIcon(imageLoader.getCardImage(aiCard)); // 상대는 보임
        } else {
